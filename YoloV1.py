...
class YoloV1(nn.Module):
    ...
    def _create_conv_layers(self, architecture):
        ...
        for x in architecture:
            if type(x) == tuple:
                ...
                # New in_channels = out channels from previous layer
                in_channels = x[1]
                ...
                conv1, conv2, num_repeats = x  # [tuple, tuple, int]
    ...
    def _create_fcs(self, split_size, num_boxes, num_classes):
        ...
        return nn.Sequential(
            nn.Flatten(),
            nn.Linear(1024 * S * S, 496),  # Paper uses 4096 and not 496
            nn.Dropout(0.0),  # Use 0.5?
            nn.LeakyReLU(0.1),
            # Num Classes + Num Bounding Boxes * 5 (since, each box has a probability and 4 values associated with it)
            nn.Linear(496, S * S * (C + B * 5))
            # As per paper, putting all values gives C + B*5 = 30
        )
...
def test(split_size=7, num_boxes=2, num_classes=20):
    ...
    assert model(x).shape == torch.Size((2, 1470)) # 7 * 7 * 30 = 1470
